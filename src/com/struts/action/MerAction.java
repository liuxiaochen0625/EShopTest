...
public class MerAction extends BaseAction {
	...
	public ActionForward browseIndexMer(ActionMapping mapping, ActionForm form,
		...
		try{
			...
			topmerList = service.browseMer(paseSize,pageNo,-1,true);
			...
			if (topmerList!=null)request.setAttribute("topmerList", topmerList);
		}catch(Exception ex){
	...
	public ActionForward browseTopMer(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) {
		List merList = null;
		MerService service = new MerServiceImpl();
		int pageNo = 1; //页号
		...
		if (request.getParameter("pageNo")!=null)pageNo = Integer.parseInt(request.getParameter("pageNo"));			
		try{
			merList = service.browseMer(pageSize,pageNo,-1,false);
			totals = service.countRecord("select count(*) from Merchandise");
			if (merList!=null)request.setAttribute("merList", merList);
			//设置总记录数、总页数、当前页号
			totalPages = totals / pageSize;
			if ((totals % pageSize)>0) totalPages++; 
